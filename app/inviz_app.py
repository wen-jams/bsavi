        ...
        if not index:
            ...
            for element in new_index:
                ...
                for key in observables_keys:
                    ...
                    hv_element = getattr(hv, 'Curve')
                    plot_observable = hv_element(dataset, kdim, vdim, label=key).opts(framewise=True)
...
viz(data=df, myfunction=example_observables, myfunction_args=(df,), show_observables=True, latex_dict=latex_dict, curve_opts=exopts).servable('Waveforms')